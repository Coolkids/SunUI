...
local function gen_hpstrings(f, unit)
    ...
    h:SetFrameLevel(10)
    f.tagname = gen_fontstring(h, DB.Font, U["FontSize"], "THINOUTLINE")
	f.taghp = gen_fontstring(h, DB.Font,U["FontSize"], "THINOUTLINE")
	f.tagpp = gen_fontstring(h, DB.Font, (U["FontSize"]), "THINOUTLINE")
    f.taginfo = gen_fontstring(h, DB.Font, U["FontSize"], "THINOUTLINE")
...
local function gen_castbar(f)
	...
	local sp
    --spell text
    local txt = gen_fontstring(s, DB.Font, (U["FontSize"]+1)*1, "THINOUTLINE")
    ...
    --time
    local t = gen_fontstring(s, DB.Font, (U["FontSize"]+1)*1, "THINOUTLINE")
    ...
    elseif f.mystyle == "player" then
		...
		local l = gen_fontstring(s, DB.Font, U["FontSize"]*1, "THINOUTLINE")
...
local function PostCreateIcon(self, button)
    ...
    h3:SetFrameLevel(10)
	button.count = gen_fontstring(h3, DB.Font, (U["FontSize"]-1)*1, "THINOUTLINE")
    ...
    h2:SetFrameLevel(10)
    button.remaining = gen_fontstring(h2, DB.Font, (U["FontSize"]-1)*1, "THINOUTLINE")
...
local function gen_EclipseBar(f)
	...
	eb:SetPoint('BOTTOMLEFT', f, 'TOPLEFT', 0, 2)
	...
	f.EclipseBar = eb
	local ebInd = gen_fontstring(h, DB.Font, (U["FontSize"]+4)*1, "THINOUTLINE")
...
local function gen_arenatracker(f)
	...
	at.text = gen_fontstring(at, DB.Font, (U["FontSize"]-1)*1, "THINOUTLINE")
...
function UF:fixArena()
	...
	for i = 1, 5 do
		...
		arenaprep[i].Spec = gen_fontstring(arenaprep[i].Health, DB.Font, U["FontSize"]*1, "THINOUTLINE")
