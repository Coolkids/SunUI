...
local Freebgrid_OnEvent = function(self, event, ...)
	...
	elseif ( arg1 == self.unit or arg1 == self.displayedUnit or (arg1 == "vehicle" and self.inVehicle)) then
		...
		elseif ( event == "UNIT_MAXPOWER" or  event == "UNIT_POWER" or event == "UNIT_DISPLAYPOWER" or event == "UNIT_POWER_BAR_SHOW" or event == "UNIT_POWER_BAR_HIDE") then
			ns:UpdatePowerBarLayout(self)
			ns:UpdatePower(self)
...
function ns:UpdateBlizzardPartyFrameDisplayStatus()
	if not ns.db.hideblzparty then
		ns:UpdateBlizzardCompactRaidFrameManager()
...
local function UpdateBlizzardPartyFrame(self)
...
function ns:UpdatePowerBarLayout(self) 
	local power = self.PowerBar
...
function ns:UpdatePowerBar(self) 
	...
	if not ns.db.powerbar then return end
...
local OnEvent = function(self, event, ...)
	...
	elseif ( event == "PLAYER_LOGIN" ) then
		...
		hooksecurefunc("PartyMemberFrame_OnUpdate", UpdateBlizzardPartyFrame)
