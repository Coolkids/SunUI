...
local function LoadSkin()
	A:SetBD(SpellBookFrame)
	A:ReskinArrow(SpellBookPrevPageButton, "left")
	A:ReskinArrow(SpellBookNextPageButton, "right")
	A:Reskin(SpellBookCompanionSummonButton)
	A:ReskinClose(SpellBookFrameCloseButton)
	...
	for i = 1, 5 do
		A:CreateTab(_G["SpellBookFrameTabButton"..i])
	end
	...
	for _, button in pairs(professions) do
		...
		bu.statusBar:SetStatusBarTexture(A["media"].backdrop)
	...
	hooksecurefunc("SpellBook_UpdateCoreAbilitiesTab", function()
		for i = 1, #SpellBookCoreAbilitiesFrame.Abilities do
			...
			if not bu.reskinned then
				...
				bu.iconTexture.bg = A:CreateBG(bu.iconTexture)
