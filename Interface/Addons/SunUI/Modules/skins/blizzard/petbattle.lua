...
local function LoadSkin()
	...
	for index, infoBar in pairs(infoBars) do
		...
		infoBar.PetTypeFrame.text:SetFont(S["media"].font, S["media"].fontsize, "OUTLINE")
		infoBar.PetTypeFrame.text:SetShadowColor(0, 0, 0, 1)
		infoBar.PetTypeFrame.text:SetShadowOffset(S.mult, -S.mult)
		infoBar.PetTypeFrame.text:SetText("")

		infoBar.ActualHealthBar:ClearAllPoints()
		infoBar.Name:ClearAllPoints()
		infoBar.Name:SetFont(S["media"].font, S["media"].fontsize, "OUTLINE")
	...
	hooksecurefunc("PetBattleAuraHolder_Update", function(self)
		...
		for i=1, C_PetBattles.GetNumAuras(self.petOwner, self.petIndex) do
			...
			if (isBuff and self.displayBuffs) or (not isBuff and self.displayDebuffs) then
				...
				frame.Duration:SetFont(S["media"].font, S["media"].fontsize, "OUTLINE")
				frame.Duration:SetShadowColor(0, 0, 0, 1)
				frame.Duration:SetShadowOffset(S.mult, -S.mult)
	...
	local function SkinPetTooltip(tt)
		...
		tt.border:SetBackdropBorderColor(unpack(S["media"].bordercolor))
		tt.shadow:SetBackdropBorderColor(unpack(S["media"].bordercolor))
	end
	...
	A:Reskin(bf.TurnTimer.SkipButton)
