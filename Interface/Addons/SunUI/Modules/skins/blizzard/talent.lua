...
local function LoadSkin()
	...
	A:CreateBG(PlayerTalentFrameTalentsClearInfoFrame)
	...
	for _, name in pairs(buttons) do
		for i = 1, 4 do
			...
			_G["PlayerTalentFrameSpecializationSpecButton"..i.."Glow"].Show = S.dummy
			
			A:Reskin(bu, true)
			
			bu.selectedTex:SetTexture("")
			bu.learnedTex:SetTexture(A["media"].backdrop)
	...
	A:ReskinPortraitFrame(PlayerTalentFrame, true)
	A:Reskin(PlayerTalentFrameSpecializationLearnButton)
	A:Reskin(PlayerTalentFrameTalentsLearnButton)
	A:Reskin(PlayerTalentFrameActivateButton)
	A:Reskin(PlayerTalentFramePetSpecializationLearnButton)
end

A:RegisterSkin("Blizzard_TalentUI", LoadSkin)