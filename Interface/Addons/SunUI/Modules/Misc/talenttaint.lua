...
local T = LibStub("AceAddon-3.0"):GetAddon("SunUI"):NewModule("talentTaint", "AceEvent-3.0", "AceHook-3.0")

local BlizzardStaticPopupDialogs = {
...
function T:FCF_StartAlertFlash(chatFrame)
...
function T:FCF_StopAlertFlash(chatFrame)
	if ( chatFrame.minFrame ) then
...
function T:ADDON_LOADED(event, addon)
		 ...
		 for i=1, 10 do
            ...
            tab:SetScript("PreClick", function()
                for index = 1, STATICPOPUP_NUMDIALOGS, 1 do
                    ...
                    if frame:IsShown() and not issecurevariable(frame, "which") then
