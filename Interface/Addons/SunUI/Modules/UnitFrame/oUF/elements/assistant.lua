...
local Update = function(self, event)
	...
	local isAssistant = IsInRaid(unit) and (UnitIsGroupLeader(unit) or UnitIsGroupAssistant(unit)) --UnitInRaid(unit) (WoW5 and UnitIsGroupAssistant(unit) or UnitIsRaidOfficer(unit)) and not (WoW5 and UnitIsGroupLeader(unit) or UnitIsPartyLeader(unit))
	if(isAssistant) then
		assistant:Show()
	else
		assistant:Hide()
