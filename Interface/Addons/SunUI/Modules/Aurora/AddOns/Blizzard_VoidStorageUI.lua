...
local updateContents = function()
	...
	for slot = 1, VOID_WITHDRAW_MAX or 80 do
		S.ColourQuality(_G["VoidStorageStorageButton"..slot], GetVoidItemInfo(slot))
	end

	for slot = 1, VOID_WITHDRAW_MAX or 9 do
		S.ColourQuality(_G["VoidStorageWithdrawButton"..slot], GetVoidTransferWithdrawalInfo(slot))
	end
end
...
local updateDeposit = function(slot)
	...
	S.ColourQuality(_G["VoidStorageDepositButton"..slot], GetVoidTransferDepositInfo(slot))
end
