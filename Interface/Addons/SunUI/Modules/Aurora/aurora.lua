...
Skin:SetScript("OnEvent", function(self, event, addon)
	...
	if addon=="SunUI" then 
		...
		hooksecurefunc("XUIDropDownMenu_CreateFrames", function(level, index)
			for i = 1, UIDROPDOWNMENU_MAXLEVELS do
				local menu = _G["XDropDownList"..i.."MenuBackdrop"]
				local backdrop = _G["XDropDownList"..i.."Backdrop"]
				if not backdrop.reskinned then
					S.CreateBD(menu)
					S.CreateBD(backdrop)
					backdrop.reskinned = true
		...
		hooksecurefunc("XToggleDropDownMenu", function(level, _, dropDownFrame, anchorName)
			if not level then level = 1 end
			...
			for j = 1, UIDROPDOWNMENU_MAXBUTTONS do
				local bu = _G["XDropDownList"..level.."Button"..j]
				if bu == nil then return end
				local _, _, _, x = bu:GetPoint()
				if bu:IsShown() and x then
					local hl = _G["XDropDownList"..level.."Button"..j.."Highlight"]
					local check = _G["XDropDownList"..level.."Button"..j.."Check"]
					...
					if not bu.bg then
						createBackdrop(bu, check)
						hl:SetTexture(r, g, b, .2)
						_G["XDropDownList"..level.."Button"..j.."UnCheck"]:SetTexture("")
					end
