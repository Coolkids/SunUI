...
local XCT = S:GetModule("xCT")

--some init
--local addon, ns = ...
local ct = XCT.config
ct.myname = S.myname
ct.myclass = S.myclass
...
for i = 1, numf do
	...
	f:SetShadowOffset((S.mult or 1), -(S.mult or 1))
    ...
    if framenames[i] == "dmg" then
        ...
        f:SetPoint(unpack(ct.xdmg))
		f:SetSize(ct.xdmgwidth, ct.xdmgheight)
    elseif framenames[i] == "heal" then
        f:SetJustifyH(ct.justify_2)
		f:SetPoint(unpack(ct.xheal))
		f:SetSize(ct.xhealwidth, ct.xhealheight)
    elseif framenames[i] == "gen" then
        f:SetJustifyH(ct.justify_3)
        f:SetSize(ct.xgenwidth, ct.xgenheight)
        f:SetPoint(unpack(ct.xgen))
...
if not ct.blizzheadnumbers then
	...
	fsTitle:SetShadowColor(0, 0, 0, 1)		
	fsTitle:SetShadowOffset((S.mult or 1), -(S.mult or 1))
	...
	fsVersion:SetShadowColor(0, 0, 0, 1)
	fsVersion:SetShadowOffset((S.mult or 1), -(S.mult or 1))
...
local StartConfigmode = function()
    if not InCombatLockdown() then
        for i = 1, #ct.frames do
			...
			f.fs:SetShadowOffset((S.mult or 1), -(S.mult or 1))
			...
			f.fsp:SetShadowColor(0, 0, 0, 1)		
			f.fsp:SetShadowOffset((S.mult or 1), -(S.mult or 1))
			...
			f.fsw:SetShadowColor(0, 0, 0, 1)
			f.fsw:SetShadowOffset((S.mult or 1), -(S.mult or 1))
			...
			f.fsh:SetShadowColor(0, 0, 0, 1)		
			f.fsh:SetShadowOffset((S.mult or 1), -(S.mult or 1))
...
if(ct.damageout)then
    ...
    if ct.icons then
        ct.blank = "Interface\\BUTTONS\\WHITE8X8"
    end
...
function XCT:init()
	...
	-- Register DK Events
	if ct.dkrunes and S.myclass == "DEATHKNIGHT" then
