...
function stAddonManager:LoadProfileWindow()
	...
	local EnableAll = CreateMenuButton(window, (window:GetWidth()-15)/2, 20, "全部启用", "TOPLEFT", window, "BOTTOMLEFT", 5, -5)
	...
	local DisableAll = CreateMenuButton(window, EnableAll:GetWidth(), EnableAll:GetHeight(), "全部禁用", "TOPRIGHT", window, "BOTTOMRIGHT", -5, -5)
	...
	local SaveProfile = CreateMenuButton(window, window:GetWidth()-10, 20, "新建配置文件", "TOPLEFT", EnableAll, "BOTTOMLEFT", 0, -5)
	SaveProfile:SetScript("OnClick", function(self)
		if not self.editbox then
			...
			editbox:SetScript("OnEscapePressed", function(self) self:SetText("配置名") self:ClearFocus() self:Hide() end)
	...
	function stAddonManager:UpdateProfileList()
		...
		for profileName, addonList in sort(stAddonProfiles, function(a, b) return strlower(b) > strlower(a) end) do
			if not buttons[i] then
				local button = CreateMenuButton(window, window:GetWidth()-10, 20, "<".."配置名"..">")
				...
				overlay.set = CreateMenuButton(overlay, overlay:GetWidth(), 20, "设置给", "TOP", button, "TOP", 0, -18)
				overlay.add = CreateMenuButton(overlay, overlay:GetWidth(), 20, "添加", "TOP", overlay.set, "BOTTOM", 0, 1)
				overlay.remove = CreateMenuButton(overlay, overlay:GetWidth(), 20, "移除", "TOP", overlay.add, "BOTTOM", 0, 1)
				overlay.delete = CreateMenuButton(overlay, overlay:GetWidth(), 20, "删除配置文件", "TOP", overlay.remove, "BOTTOM", 0, 1)
			...
			overlay.delete:SetScript("OnClick", function(self)
				if IsShiftKeyDown() then
					...
					print("|cff00aaffAddonManager|r: 删除完毕")
				end
			end)
			i = i + 1
		end
...
function stAddonManager:LoadWindow()
	...
	close:SetScript("OnClick", function() S:FadeOutFrame(window, 0.3) end)
	header.close = close
	...
	for i, addon in pairs(stAddonManager.AllAddons) do
		...
		button.Icon:SetTexture("Interface\\ChatFrame\\ChatFrameBackground") 
		...
		if i == 1 then
			button:SetPoint("TOPLEFT", scrollFrame.Anchor, "TOPLEFT", 7, -5)
		else
		...
		button:SetScript("OnEnter", function(self)
			...
			if addon.dependencies then GameTooltip:AddLine("依赖于"..unpack(addon.dependencies), 1, .5, 0) end
			if addon.optionaldependencies then GameTooltip:AddLine("可选依赖"..unpack(addon.optionaldependencies), 1, .5, 0) end
			GameTooltip:AddLine(" ")
			GameTooltip:AddDoubleLine("内存占用", FormatMemory(mem), 1, 1, 1, r, g, b)
			GameTooltip:Show()
		end)
	...
	SkinFrame(searchBar)
	searchBar:FontTemplate()
	searchBar:SetText("查找")
	...
	searchBar:SetScript("OnEscapePressed", function(self) searchBar:SetText("查找") stAddonManager:UpdateList(stAddonManager.AllAddons) searchBar:ClearFocus() end)
	...
	sbClear:SetScript("OnClick", function(self) searchBar:SetText("查找") stAddonManager:UpdateList(stAddonManager.AllAddons) searchBar:ClearFocus() end)
	searchBar.clear = sbClear
	stAddonManager.searchBar = searchBar

	local profileButton = CreateMenuButton(window, 50, 20, "配置文件", "TOPRIGHT", header, "BOTTOMRIGHT", -10, -5)
	profileButton:SetScript("OnClick", function(self)
		stAddonManager:LoadProfileWindow()
	end)
	stAddonManager.profileButton = profileButton
	
	local reloadButton = CreateMenuButton(window, 1, searchBar:GetHeight(), "重载界面", "LEFT", searchBar, "RIGHT", 5, 0)
...
function AM:Initialize()
	...
	gmbAddOns:SetPoint("BOTTOM", GameMenuFrame, "BOTTOM", 0, 34)
	--调整菜单大小在gui.lua中
