...
FriendsMenuXP_Buttons["PROMOTE"] = {
	...
	,
	show = function (name) 
		if (GetNumSubgroupMembers() > 0 and UnitInParty(name) and UnitIsGroupLeader(name)) or (GetNumGroupMembers() > 0 and UnitInRaid(name) and UnitIsGroupAssistant(name)) then
			return 1
		end
	end
	,
