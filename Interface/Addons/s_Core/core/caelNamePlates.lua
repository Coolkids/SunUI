...
local ThreatUpdate = function(self, elapsed)
	...
	if self.elapsed >= 0.2 then
		...
		--self.healthBar.hp:SetText(self.healthBar:GetValue())
		if((self.healthBar:GetValue())/select(2, self:GetChildren():GetMinMaxValues())~=1) then
		self.healthBar.hp:SetText(math.floor((self.healthBar:GetValue())/select(2, self:GetChildren():GetMinMaxValues())*100) .."%")
		else
		self.healthBar.hp:SetText()
		end
		self.elapsed = 0
	end
end
...
local _, castbarOverlay, shieldedRegion, spellIconRegion= castBar:GetRegions() 
	...
	--hp:SetShadowOffset(1.25, -1.25)
	healthBar.hp= frame:CreateFontString()
	healthBar.hp:SetPoint("CENTER", healthBar, "CENTER", 0, 0)
	healthBar.hp:SetFont(font, fontSize*0.85, fontOutline)
	...
	spellIconRegion:SetPoint("TOPLEFT",healthBar,"TOPRIGHT",2,0)
