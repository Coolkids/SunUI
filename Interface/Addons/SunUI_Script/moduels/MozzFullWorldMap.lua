...
## Modify: Ljxx.net 修改成单文件, 方便集成
欢迎转包, 请不要删除此段信息
--]]--------------------------------------------------------------------------------------------

local BINDING_NAME_MOZZ_WM_CHECKBOX 	= "Show Unexplored Areas";
local BINDING_HEADER_MFWM = "MFWM";

local MFWM_COLORME_BLUE = "MFWM: unexplored areas will now be blue.";
local MFWM_COLORME_NOT  = "MFWM: unexplored areas will now be normal.";
local MFWM_COLORME_RAIN = "MFWM: unexplored areas custom color chosen.";

local MFWM_TRANSME_DONE = "MFWM: Transparency : ";
local MFWM_TRANSME_ERROR = "Please enter between 0.0 and 1.0 for transparency.";

local MFWM_HELP1 = "MozzFullWorldMap Commands:";
local MFWM_HELP2 = "/mfwm blue    : shows unexplored areas in blue";
local MFWM_HELP3 = "/mfwm normal  : shows unexplored areas in the normal map color";
local MFWM_HELP4 = "/mfwm rainbow : choose custom color for unexplored areas";
local MFWM_HELP5 = "/mfwm trans [0.0 to 1.0]  : controls transparency of unexplored areas. 0.0 is completely clear, 1.0 is completely opaque.";
...
if (GetLocale() == "zhCN") then
	BINDING_NAME_MOZZ_WM_CHECKBOX 	= "显示未探索的区域";
	BINDING_HEADER_MFWM = "MFWM";

	MFWM_COLORME_BLUE = "MFWM: 未探索的区域将会以蓝色显示";
	MFWM_COLORME_NOT  = "MFWM: 未探索的区域将会以正常方式显示";
	MFWM_COLORME_RAIN = "MFWM: 未探索的区域以自定颜色显示";

	MFWM_TRANSME_DONE = "MFWM: 透明度: ";
	MFWM_TRANSME_ERROR = "请输入介于 0.0 至 1.0 之间的值以设定透明度";

	MFWM_HELP1 = "MozzFullWorldMap 指令:";
	MFWM_HELP2 = "/mfwm blue    : 以蓝色显示未探索区域";
	MFWM_HELP3 = "/mfwm normal  : 以正常地图颜色显示未探索区域";
	MFWM_HELP4 = "/mfwm rainbow : 选择自定颜色以显示未探索区域";
	MFWM_HELP5 = "/mfwm trans [0.0 ~ 1.0]  : 设定未探索区域的透明度。0.0 为完全透明，1.0 为完全不透明。";
end
...
} --Updated to WoW v4.1.0 Build 14007 (2011-04-29).
...
------------------------------------------------------------------------------------------------
local MozzWorldMapShowAllCheckButton = CreateFrame("CheckButton", "MozzWorldMapShowAllCheckButton", WorldMapPositioningGuide, "OptionsCheckButtonTemplate")
MozzWorldMapShowAllCheckButton:SetSize(24,25)
MozzWorldMapShowAllCheckButton:SetScript("OnClick", function(self)
...
MozzWorldMapShowAllCheckButton:SetScript("OnShow", function(self)
	Mozz_AnchorMapOptions();
	self:SetChecked( MozzFullWorldMap.Enabled );
end)
local MozzWorldMapShowAllLabel = MozzWorldMapShowAllCheckButton:CreateFontString("MozzWorldMapShowAllLabel", "BACKGROUND", "GameFontNormalSmall")
MozzWorldMapShowAllLabel:SetJustifyH("LEFT")
MozzWorldMapShowAllLabel:SetText(BINDING_NAME_MOZZ_WM_CHECKBOX)
MozzWorldMapShowAllLabel:SetPoint("LEFT", MozzWorldMapShowAllCheckButton, "RIGHT", 0, 0)
...
function Mozz_AnchorMapOptions()
	...
	if ( WORLDMAP_SETTINGS.size == WORLDMAP_WINDOWED_SIZE ) then
		if ( Gatherer_WorldMapDisplay ) then
			MozzWorldMapShowAllCheckButton:SetPoint( "TOPLEFT", Gatherer_WorldMapDisplay, "TOPRIGHT", 5, 0 );
...
local function SetHooks()
	...
	hooksecurefunc( "WorldMapQuestShowObjectives_AdjustPosition", Mozz_AnchorMapOptions );
...
local function SetBFMHooks()
	...
	if BattlefieldMinimap_UpdateOpacity then
		hooksecurefunc("BattlefieldMinimap_UpdateOpacity", uF)
	elseif BattlefieldMinimap_SetOpacity then
		hooksecurefunc("BattlefieldMinimap_SetOpacity", uF)
	end
end
...
local MozzFullWorldMapFrame = CreateFrame("Frame", "MozzFullWorldMapFrame", UIParent)
MozzFullWorldMapFrame:SetFrameStrata("LOW")
MozzFullWorldMapFrame:SetToplevel(true)
MozzFullWorldMapFrame:SetPoint("TOPLEFT", 0, 0)
MozzFullWorldMapFrame:SetSize(1,1)
MozzFullWorldMapFrame:RegisterEvent("ADDON_LOADED")
SLASH_MFWM1 = "/mozzfullworldmap";
SLASH_MFWM2 = "/mfwm"; -- A shortcut or alias
SlashCmdList["MFWM"] = MozzFullWorldMap_Command;

MozzFullWorldMapFrame:SetScript("OnEvent", function( self, event, ... )
		...
		-- With the 4.1 patch, we're dumping the old errata table as it likely contains
		-- old world data no longer applicable since Cataclysm

		if ( not MozzFullWorldMap.version or MozzFullWorldMap.version < "v3.4.01.00" ) then
		...
		--DEFAULT_CHAT_FRAME:AddMessage(BINDING_NAME_MOZZ_WM_CHECKBOX .."( /MFWM ) |c0000FF00"..MFWM_VERSION.."|r", 0.64, 0.21, 0.93);
...
MozzFullWorldMapFrame:SetScript("OnUpdate", function(self, elapsed)
