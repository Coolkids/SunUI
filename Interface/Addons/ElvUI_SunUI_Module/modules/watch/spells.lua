local _, ns = ...
local targetdebuff = {"BOTTOM", UIParent, "BOTTOM",  227,  335}  -- 目标debuff
local playerbuff   = {"BOTTOM", UIParent, "BOTTOM", -227,  335}  -- 玩家buff
local enbuff       = {"BOTTOM", UIParent, "BOTTOM", -227,  370}  -- 玩家饰品附魔触发buff
local playercd     = {"BOTTOM", UIParent, "BOTTOM", -227,  405}  -- 玩家技能CD

--全职业设置
local alldebuff    = {"TOP",    UIParent, "TOP",     200, -157}  -- 玩家Debuff
local imbuff       = {"TOP",    UIParent, "TOP",     200, -203}  -- 玩家重要Buff
local pvpdebuff    = {"TOP",    UIParent, "TOP",     200, -249}  -- 玩家PVPDebuff
...
ns.watchers ={
	["DRUID"] = {		-- [德鲁伊XD]
		...
		},
		{
			...
			-- 血腥之爪
			{ spellID = 214995, unitID = "target", caster = "player", filter = "DEBUFF"},
			-- 夺魂咆哮
	...
	["WARRIOR"] = { 		--[战士ZS]
		...
		},
		{
			...
			-- 援护
			{ spellID = 147833, unitID = "target", caster = "all",    filter = "BUFF"  },
	...
	["DEATHKNIGHT"] = { 		--[死骑DK]
		...
		},
		{
			...
			-- 凛冬将至
			{ spellID = 211794, unitID = "target", caster = "player", filter = "DEBUFF" },
			{ spellID = 207171, unitID = "target", caster = "player", filter = "DEBUFF" },
			-- 苍白行者
	...
	["ALL"]={
		{
			...
			-- 强化奇点
			{ spellID = 189781, unitID = "player", caster = "all", filter = "DEBUFF"},
			-- 強化魔化锁链
			{ spellID = 189777, unitID = "player", caster = "all", filter = "DEBUFF"},
			...
			-- 古尔丹之怒
			{ spellID = 186362, unitID = "player", caster = "all", filter = "DEBUFF"},
			-- 束缚暗影
			...
			-- 军团之触
			{ spellID = 190400, unitID = "player", caster = "all", filter = "DEBUFF"},
			-- 军团标记
			{ spellID = 187050, unitID = "player", caster = "all", filter = "DEBUFF"},
		},
...

		{
			...
			-----------------------------职业专精--------------------------------------
			-- 辩护圣契     (奶骑)
			{ spellID = 185100, unitID = "player", caster = "player", filter = "BUFF" },
			-- 辩护圣契     (惩戒)
			{ spellID = 185102, unitID = "player", caster = "player", filter = "BUFF" },
			-- 斥战祷言     (戒律)
			{ spellID = 184912, unitID = "player", caster = "player", filter = "BUFF" },
			-- 斥战祷言     (暗牧) 
			{ spellID = 184915, unitID = "player", caster = "player", filter = "BUFF" },
			-- 黑暗之星碎片 (毁灭) 
			{ spellID = 185229, unitID = "player", caster = "player", filter = "BUFF" },
			-- 灭世者的决意 (狂暴) 
			{ spellID = 185230, unitID = "player", caster = "player", filter = "BUFF" },
			...
			-------------------------------MOP-----------------------------------------
			-- 狂妄之诅咒              (爆击, 使用)
			{ spellID = 146395, unitID = "player", caster = "player", filter = "BUFF" },
			-- 鲁克的不幸护符          (减伤, 使用)
			...
			-- 恐惧预言 
			{ spellID = 184073, unitID = "player", caster = "player", filter = "BUFF" },
			-------------------------------MOP-----------------------------------------
			-- 伊墨苏斯的净化之缚      (智力, 触发)
			...
			-- 动荡邪影制剂            (吸血, 触发)
			{ spellID = 184671, unitID = "player", caster = "player", filter = "BUFF" },
			-------------------------------MOP-----------------------------------------
			-- 傲慢之棱光囚笼          (智力, 触发)
