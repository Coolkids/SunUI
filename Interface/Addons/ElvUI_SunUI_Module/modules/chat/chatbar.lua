...
local CT = E:GetModule("Chat-SunUI")
local Parent
local buttons = {}
local Channel = {
	{"/s", "/e ", "/y"},
...
function CT:CreateChannelBar()
	...
	for i = 1, 6 do
		...
		bg:SetTexture(E["media"].blank)
		bg:SetColorTexture(Color[i][1], Color[i][2], Color[i][3])
		Button:CreateShadow()
		...
		if i == 1 then
			Button:SetPoint("TOPLEFT", ChatFrame1Tab, "TOPLEFT", 0, 3)
		else
