...
local CCD = E:NewModule("ClassCD", "AceEvent-3.0", "AceHook-3.0", "AceConsole-3.0")
			...
			group = {
				...
				args = {
					...
					ClassCDIconSize = {
						type = "input",
						name = "图标大小",
						desc = "图标大小", disabled = function(info) return not self.db.ClassCDIcon end,
						order = 2,
					...
					ClassCDWidth = {
						type = "input",
						name = "框体宽度",
						desc = "框体宽度", disabled = function(info) return self.db.ClassCDIcon end,
						order = 5,
...
function CCD:Initialize()
	...
	self:UpdateSet()
	E:CreateMover(ClassCDAnchor, "ClassCDAnchorMover", "内置CD监视", true, nil, "ALL")
end

E:RegisterModule(CCD:GetName())
