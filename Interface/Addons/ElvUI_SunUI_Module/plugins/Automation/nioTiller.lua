...
local nioTillers = E:NewModule("nioTillers", "AceEvent-3.0", "AceHook-3.0", "AceConsole-3.0")
...
local function MainbuttonOnClick(self, button)
	if button == "LeftButton" then
		if E.db["nioTillerDBC"].Shown then 
			E.db["nioTillerDBC"].Shown = false
			for i = 2, #seedIDs+1 do nioTillers.btn[i]:Hide() end
		else
			E.db["nioTillerDBC"].Shown = true
			nioTillers:ShowButtons()	
		end
	elseif button == "RightButton" and not IsModifierKeyDown() then
		if E.db["nioTillerDBC"].ShowOnlyOwned then E.db["nioTillerDBC"].ShowOnlyOwned = false					
		else E.db["nioTillerDBC"].ShowOnlyOwned = true end
		nioTillers:HideButtons()
		nioTillers:ShowButtons()
		nioTillers:RefreshItemCount()
	elseif button == "RightButton" and IsShiftKeyDown() then
		if E.db["nioTillerDBC"].ShowTooltip then E.db["nioTillerDBC"].ShowTooltip = false					
		else E.db["nioTillerDBC"].ShowTooltip = true end
	elseif button == "RightButton" and IsControlKeyDown() then
		if E.db["nioTillerDBC"].Orientation < 4 then E.db["nioTillerDBC"].Orientation = E.db["nioTillerDBC"].Orientation + 1					
		else E.db["nioTillerDBC"].Orientation = 1 end
...
local function OnEnterMain(self)
	if not E.db["nioTillerDBC"].ShowTooltip then return end
	GameTooltip:SetOwner(self)
...
function nioTillers:SetOrientation()
	for i = 2, #seedIDs+1 do
		...
		if E.db["nioTillerDBC"].Orientation == 1 then self.btn[i]:SetPoint("LEFT", self.btn[1], "RIGHT", (i-2)*(config.Normalbutton.size + 2) +4, 0)
		elseif E.db["nioTillerDBC"].Orientation == 2 then self.btn[i]:SetPoint("TOP", self.btn[1], "BOTTOM", 0, -(i-2)*(config.Normalbutton.size + 2) -2)
		elseif E.db["nioTillerDBC"].Orientation == 3 then self.btn[i]:SetPoint("RIGHT", self.btn[1], "LEFT", -(i-2)*(config.Normalbutton.size + 2) -4, 0)
		elseif E.db["nioTillerDBC"].Orientation == 4 then self.btn[i]:SetPoint("BOTTOM", self.btn[1], "TOP", 0, (i-2)*(config.Normalbutton.size + 2) +2) end
	end
end
...
function nioTillers:MakeButtons()
	...
	holder:SetPoint(E.db["nioTillerDBC"].Position.pos, UIParent, E.db["nioTillerDBC"].Position.position, E.db["nioTillerDBC"].Position.xoff, E.db["nioTillerDBC"].Position.yoff) 
	...
	holder:SetScript("OnMouseUp", function(self, button)
		...
		E.db["nioTillerDBC"].Position.pos, E.db["nioTillerDBC"].Position.position, E.db["nioTillerDBC"].Position.xoff, E.db["nioTillerDBC"].Position.yoff =  AnchorF,AnchorT, X, Y
	end)
...
function nioTillers:ShowButtons()	
	...
	for i,v in ipairs(seedIDs) do		
		...
		if (count > 0 and E.db["nioTillerDBC"].ShowOnlyOwned) or not E.db["nioTillerDBC"].ShowOnlyOwned then
...
function nioTillers:Enable()
	...
	SetMainButton(E.db["nioTillerDBC"].LastSeed, nioTillers.SAButtons[1], nioTillers.btn[1], GetItemIcon(E.db["nioTillerDBC"].LastSeed))
...
local function nioTillersEvents(self, event)
	...
	elseif event == "ADDON_LOADED" then
		if not E.db["nioTillerDBC"] then E.db["nioTillerDBC"] = default_options end
		if E.db["nioTillerDBC"].Position == nil then E.db["nioTillerDBC"] = default_options end
...
E:RegisterModule(nioTillers:GetName())
